...
// import { getPersetujuanQuotation, updatePersetujuanQuotation } from "./service";

// export const getPersetujuan = async (req: MyRequest, res: Response, next: NextFunction) => {
//     try {
//         const id_pegawai = req.user?.id as number;

//         const data = await getPersetujuanQuotation(id_pegawai);
//         res.status(200).json({
...
//         }
//         const data = await updatePersetujuanQuotation(parseInt(req.body.id), req.user?.id as number, req.body.status, req.body.keterangan);
//         res.status(200).json({

...
import { NewKontak, Kontak, kontak, KategoriKontak, NewKategoriKontak, kategoriKontak, kontakColumns } from "./schema";
...
export const getKontakOption = async (option: any, tx = db) => {
    ...
    if (option.is_supplier) {
        ...
        if (option.is_hutang) {
            data.innerJoin(
                tx
                    .select({ id_supplier: acc_ap_faktur.id_supplier, sisa: sum(sql`amount-discount-pay`).as("sisa") })
                    .from(acc_ap_faktur)
                    .where(lt(sql`pay+discount`, sql`amount`))
                    .groupBy(acc_ap_faktur.id_supplier)
                    .as("a"),
...
export const getCustomerPegawai = async () => {
    const dataCustomer = await db.execute(sql`SELECT 'C' as jenis, id, kontak as nama FROM kontak WHERE is_aktif = true AND is_customer = true `);

    const dataPegawai = await db.execute(sql`SELECT 'C' as jenis, id,  nama FROM hr.pegawai`);

    const data = dataCustomer.concat(dataPegawai);
    return data;
};

...
import { type NewHariLiburTipe, type HariLiburTipe, hari_libur_tipe, UpdateHariLiburTipe } from "@/databases/hrd/hari-libur/tipe/schema";
...
export const getHariLiburTipeById = async (params: HariLiburTipe["id"], tx = db) => {
    const [data] = await tx.select().from(hari_libur_tipe).where(eq(hari_libur_tipe.id, params));
    return data;
};
...
export const updateHariLiburTipe = async (params: HariLiburTipe["id"], form: UpdateHariLiburTipe, tx = db) => {

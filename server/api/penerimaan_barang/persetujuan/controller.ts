...
import { getPersetujuanPenerimaanBarang, updatePersetujuanPenerimaanBarang } from "./service";
...
export const getPersetujuan = async (req: MyRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const data = await getPersetujuanPenerimaanBarang(id_pegawai);
        res.status(200).json({
...
export const updatePersetujuan = async (req: MyRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const data = await updatePersetujuanPenerimaanBarang(parseInt(req.body.id), req.user?.id as number, req.body.status, req.body.keterangan);
        res.status(200).json({
